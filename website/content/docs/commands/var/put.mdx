...
containing characters outside the set `[a-zA-z0-9_]` can not be read directly
using dotted references by the template engine. Instead, they require the use of
the `index` template function to directly access their values. This does not
impact cases where the keys and values are read using the `range` function.

Variable items are restricted to 16KiB in size. This limit is calculated by
taking the sum of the length in bytes of all the unencrypted keys and values.
