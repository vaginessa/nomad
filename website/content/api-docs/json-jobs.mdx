...
- `Identity` - Specifies whether or not to expose Nomad's [Workload Identity][]
  to the task. If non-null the `NOMAD_TOKEN` environment variable and the
  `secrets/nomad_token` file will contain the task's workload identity token.

  - `Env` - If set to `false` the `NOMAD_TOKEN` environment variable will not
    be set.

  - `File` - If set to `false` the `secrets/nomad_token` file will not be
    created.
...
[Workload Identity]: /docs/concepts/workload-identity 'Nomad Workload Identity'
