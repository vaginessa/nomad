...
nomad var put [options] <path to store variable> [<variable spec file reference>] [<key>=<value>]...
  ...
  specified.
	...
	Typically Items are consumed by Nomad job specifications using the internal
	consul-template library. Non-alphanumeric keys [A-Za-z0-9_] require the use
	of the ` + "`" + `index` + "`" + `function to read by name. Ranging over non-alpha-
	numeric keys is not impacted.
...
func (c *VarPutCommand) printWarnings(diags []*parseIdentifierDiag) {
	var lineLen int = 20
	if c.ErrDimensions.Width > 20 {
		lineLen = c.ErrDimensions.Width
	}
	if c.ErrDimensions.Err != nil {
		lineLen = 80
	}
